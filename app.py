...
app.layout = html.Div([
    html.Div([
        html.Div([
            html.H1("Monitoramento de Alertas"),
            html.P("Prefeitura Municipal de Envira - BI"),
    ...
    html.Div([
        html.Div([
            html.Div([
                html.Div(className='filter-label', children='Período Início'),
                dcc.DatePickerSingle(id='start-date', date=datetime(2025, 8, 13), display_format='DD/MM/YYYY'),
            ], className='filter-group'),

            html.Div([
                html.Div(className='filter-label', children='Período Fim'),
                dcc.DatePickerSingle(id='end-date', date=datetime(2026, 5, 7), display_format='DD/MM/YYYY'),
            ], className='filter-group'),

            html.Div([
                html.Div(className='filter-label', children='Secretaria'),
                dcc.Dropdown(id='secretaria-dropdown', multi=True, placeholder="Todas", style={'minWidth': '200px'}),
            ], className='filter-group'),

            html.Div([
                html.Div(className='filter-label', children='Status'),
                dcc.Dropdown(id='status-dropdown', multi=True, placeholder="Todos", style={'minWidth': '200px'}),
            ], className='filter-group'),

            html.Button('🔄 Atualizar', id='refresh-btn', n_clicks=0, className='refresh-btn'),
    ...
    html.Div([
        html.Div([
            html.Div([
                ...
                html.Div(className='metric-label', children='Total de Alertas'),
            ], className='metric-header'),
            html.Div(className='metric-value', id='total-alertas', children="0"),
        ], className='metric-card'),

        html.Div([
            html.Div([
                html.Div(className='metric-icon', children='⏳'),
                html.Div(className='metric-label', children='Em Triagem'),
            ], className='metric-header'),
            html.Div(className='metric-value gold', id='em-triagem', children="0"),
        ], className='metric-card gold'),

        html.Div([
            html.Div([
                html.Div(className='metric-icon', children='🏆'),
                html.Div(className='metric-label', children='Secretaria Líder'),
            ], className='metric-header'),
            html.Div(className='metric-value gold', id='secretaria-lider', children="-"),
        ], className='metric-card gold'),

        html.Div([
            html.Div([
                html.Div(className='metric-icon', children='📈'),
                html.Div(className='metric-label', children='Taxa Triagem'),
            ], className='metric-header'),
            html.Div(className='metric-value gold', id='taxa-triagem', children="0%"),
        ], className='metric-card gold'),
...
def update_dropdowns(data):
    secretarias = [{'label': s, 'value': s} for s in sorted(df_global['secretaria'].unique())]
    status = [{'label': s, 'value': s} for s in sorted(df_global['current_status'].dropna().unique())]
...
def update_dashboard(start_date, end_date, secretarias, status, data, n_clicks):
    df = df_global.copy()
    ...
    ranking = df.groupby('secretaria').size().reset_index(name='total').sort_values('total', ascending=False)
    ...
    color_palette = ['#14622C', '#F2A426', '#38a169', '#dd6b20', '#e53e3e', '#2b6cb0', '#1a365d', '#718096']
    fig_pie = px.pie(
